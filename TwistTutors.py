...
async def help(ctx, subject):
    if subject.lower() == "math" or subject.lower() == "m":
        ...
        if language_wait_for.content == "chinese":
            await channel.send(f"@Chinese Tutor {ctx.author.mention} needs help with chinese homework!")
        elif language_wait_for.content == "spanish":
            await channel.send(f"@Spanish Tutor {ctx.author.mention} needs help with spanish homework!")
        else:
